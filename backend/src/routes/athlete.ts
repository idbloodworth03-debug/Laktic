...
router.patch('/profile', auth, requireAthlete, async (req: AuthRequest, res) => {
  const { data, error } = await supabase
    .from('athlete_profiles')
    .update(req.body)
...
router.get('/season', auth, requireAthlete, async (req: AuthRequest, res) => {
...
router.patch('/season/races', auth, requireAthlete, async (req: AuthRequest, res) => {
...
router.post('/subscribe/:botId', auth, requireAthlete, async (req: AuthRequest, res) => {
  ...
  if (existingSeason) {
    return res.status(400).json({ error: 'Already subscribed. Use Regenerate Plan to update your schedule.' });
  }
...
router.post('/season/regenerate', auth, requireAthlete, async (req: AuthRequest, res) => {
  const { data: season } = await supabase
    .from('athlete_seasons')
...
router.get('/chat', auth, requireAthlete, async (req: AuthRequest, res) => {
  const { data: season } = await supabase
    .from('athlete_seasons')
    .select('id')
    .eq('athlete_id', req.athlete.id)
...
router.post('/chat', auth, requireAthlete, async (req: AuthRequest, res) => {
  ...
  if (planWasUpdated && planUpdates) {
    ...
    for (const update of planUpdates) {
      ...
      const dayIdx = updatedPlan[weekIdx].workouts.findIndex((wo: any) => wo.day_of_week === update.day_of_week);
