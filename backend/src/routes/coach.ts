...
router.post('/profile', auth, async (req: AuthRequest, res) => {
...
// GET /api/coach/bot
router.get('/bot', auth, requireCoach, async (req: AuthRequest, res) => {
...
// POST /api/coach/bot
router.post('/bot', auth, requireCoach, async (req: AuthRequest, res) => {
...
// PATCH /api/coach/bot
router.patch('/bot', auth, requireCoach, async (req: AuthRequest, res) => {
  const { data: bot } = await supabase.from('coach_bots').select('id, is_published').eq('coach_id', req.coach.id).single();
  if (!bot) return res.status(404).json({ error: 'Bot not found' });

  if (req.body.is_published === false && bot.is_published) {
    const { data: seasons } = await supabase.from('athlete_seasons').select('id').eq('bot_id', bot.id).eq('status', 'active');
...
router.post('/bot/workouts', auth, requireCoach, async (req: AuthRequest, res) => {
  ...
  const { data, error } = await supabase
    .from('bot_workouts')
    .upsert({ bot_id: bot.id, day_of_week, title, description, distance_miles, pace_guideline, ai_adjustable },
...
router.delete('/bot/workouts/:day', auth, requireCoach, async (req: AuthRequest, res) => {
  const { data: bot } = await supabase.from('coach_bots').select('id').eq('coach_id', req.coach.id).single();
  if (!bot) return res.status(404).json({ error: 'Bot not found' });
...
router.post('/bot/publish', auth, requireCoach, async (req: AuthRequest, res) => {
  const { data: bot } = await supabase
    .from('coach_bots')
    .select('*')
    .eq('coach_id', req.coach.id)
  ...
  if (!bot.event_focus && !bot.level_focus) return res.status(400).json({ error: 'Set at least one focus (event or level).' });

  const { data: workouts } = await supabase.from('bot_workouts').select('id').eq('bot_id', bot.id);
  const { data: knowledge } = await supabase.from('coach_knowledge_documents').select('id').eq('coach_bot_id', bot.id);

  if (!workouts || workouts.length < 5 || !knowledge || knowledge.length < 1) {
    return res.status(400).json({ error: 'Add at least 5 workouts and one knowledge document before publishing.' });
  }
...
// Knowledge Documents
router.get('/bot/knowledge', auth, requireCoach, async (req: AuthRequest, res) => {
  const { data: bot } = await supabase.from('coach_bots').select('id').eq('coach_id', req.coach.id).single();
...
router.post('/bot/knowledge', auth, requireCoach, async (req: AuthRequest, res) => {
  const { data: bot } = await supabase.from('coach_bots').select('id').eq('coach_id', req.coach.id).single();
  if (!bot) return res.status(404).json({ error: 'Bot not found' });

  const { title, document_type, content_text, source_file_name } = req.body;
  if (!content_text || content_text.length > 20000) {
    return res.status(400).json({ error: 'content_text required and must be under 20,000 characters' });
  }
...
router.patch('/bot/knowledge/:id', auth, requireCoach, async (req: AuthRequest, res) => {
  const { data, error } = await supabase
    .from('coach_knowledge_documents')
    .update({ ...req.body, updated_at: new Date().toISOString() })
    .eq('id', req.params.id)
...
router.delete('/bot/knowledge/:id', auth, requireCoach, async (req: AuthRequest, res) => {
