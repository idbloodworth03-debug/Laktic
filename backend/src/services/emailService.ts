 ...
 *
 * BLOCKED: requires RESEND_API_KEY environment variable.
 * Set RESEND_API_KEY in .env to enable email delivery.
 * All sends are no-ops (logged only) until the key is present.
 */
...
async function send(to: string, subject: string, html: string): Promise<void> {
  ...
  if (!key) {
    console.warn(`[emailService] BLOCKED: RESEND_API_KEY not set — skipping email to ${to} (subject: "${subject}")`);
    return;
  }
...
<div class="wrap">
  ...
  <div class="step"><span class="step-num">2</span><strong>Connect Strava</strong> — So your plan adapts based on your actual runs.</div>
  <div class="step"><span class="step-num">3</span><strong>Add your race calendar</strong> — Your season plan is built around your goal race.</div>
  ...
  <a href="${process.env.FRONTEND_URL}/athlete/onboarding" class="btn">Get My Plan →</a>
