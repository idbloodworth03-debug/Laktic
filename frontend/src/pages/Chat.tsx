...
export function Chat() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Subheader */}
      <div className="border-b border-[var(--border)] bg-[var(--surface)] px-6 py-2.5 flex items-center justify-between">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 max-w-3xl w-full mx-auto">
        ...
        {messages.length === 0 && !sending && (
          <div className="text-center py-16 text-[var(--muted)]">
            <div className="w-14 h-14 rounded-full bg-[var(--surface2)] border border-[var(--border2)] flex items-center justify-center mx-auto mb-4 text-2xl">💬</div>
            <p className="text-sm leading-relaxed max-w-xs mx-auto">
              Send a message to your coach bot. Ask about training, request modifications, or discuss upcoming races.
            </p>
            <div className="mt-5 flex flex-wrap gap-2 justify-center">
              {SUGGESTIONS.map(s => (
                <button key={s} onClick={() => setInput(s)}
                  className="text-xs px-3 py-1.5 rounded-full border border-[var(--border)] text-[var(--muted)] hover:text-[var(--text2)] hover:border-[var(--border2)] transition-all">
                  {s}
                </button>
              ))}
            </div>
      ...
      <div className="border-t border-[var(--border)] bg-[var(--surface)] px-6 py-4">
        <div className="max-w-3xl mx-auto flex gap-3 items-end">
          <textarea
            ...
            className="flex-1 bg-[var(--surface2)] border border-[var(--border)] rounded-xl px-4 py-3 text-sm text-[var(--text)] placeholder-[var(--muted2)] focus:outline-none focus:border-brand-500/50 focus:ring-2 focus:ring-brand-500/10 transition-all resize-none min-h-[44px] max-h-32"
            style={{ height: 'auto' }}
        ...
        <div className="max-w-3xl mx-auto mt-1.5">
          <p className="text-xs text-[var(--muted)]">
            ...
            <Link to="/athlete/races" className="text-brand-400 hover:text-brand-300 transition-colors">Regenerate Plan</Link>.
          </p>
        </div>
      </div>
    </div>
